...
// bufferFlag为1是启动控制台缓存
void setBufferFlag(int flag){
   bufferFlag = flag;
}

// showAtonce为1时，从标准输入中得到的字符立即显示在屏幕上
...
cgaputc(int c)
{
  ...
  // }
  if((pos/80) > 24){  // Scroll up.   把屏幕从24行扩充到25行
...
void setCursorPos(int x, int y) {
  if (x < 0 || x > 24 || y < 0 || y > 79){
	cprintf("(%d,%d)",x,y);
	panic("In console.c(setCursorPos(int,int) ), x or y under/overflow\n");
  }
...
// 要求： 0 <= pos < 25*80
void copyFromTextToScreen(char* text,int pos,int len){
   char *p = text;
